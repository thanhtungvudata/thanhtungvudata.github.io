...
Suppose: 
- $$\mu_A$$ and $$\sigma_A$$ is the true average time on page and standard deviation in A
- $$\mu_B$$ and $$\sigma_B$$ is the true average time on page and standard deviation in B
...
In simple terms, SE tells you how much "wiggle room" you should expect in your A/B test results — even if there were actually no real difference in the true average time on page of A and B.
...
- **Comparing Proportions Directly**: The t-test is designed for continuous outcomes (means). If you’re comparing proportions (like conversion rates), it's more appropriate to use a z-test (for larger samples) or an exact test (for very small samples), or apply suitable transformations.
- **Severely Non-Normal Data in Very Small Samples**: When the data is heavily skewed or not normally distributed, and the sample size is extremely small, the t-test may not be reliable. In such cases, consider non-parametric alternatives like the Mann-Whitney U test.
