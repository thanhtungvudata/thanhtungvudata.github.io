...
Computing $$U_A$$ using the above definition requires $$n_A n_B$$ comparison, which has time complexity of $$\mathcal{O}(n_A n_B)$$ and becomes very expensive if the sample sizes are large. Therefore, we will use a different way that is significantly more computationally efficient:

$$U_A = R_{A} - \frac{n_A (n_A+1)}{2}$$

where $$R_A = \sum_{i=1}^{n_A} \text{rank}(X_i)$$, and $$\text{rank}(X_i)$$ is the rank of $X_i$ in the joint list of both groups A and B. 
