...
$$H_0: P(X \leq Y) = P(Y \leq X), \,\, \text{(two-sided test)}$$
...
The Mann–Whitney U statistic counts the number of times an observation from one group is less than an observation from the other group:
