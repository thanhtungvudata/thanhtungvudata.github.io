...
Computing $$U_A$$ or $$U_B$$ using the above definition requires $$n_A n_B$$ comparison, which has time complexity of $$\mathcal{O}(n_A n_B)$$ and becomes very expensive if the sample sizes are large. Therefore, we will use a different way that is significantly more computationally efficient:

$$U_A = R_{A} - \frac{n_A (n_A+1)}{2}$$

$$U_B = R_{B} - \frac{n_B (n_B+1)}{2}$$
