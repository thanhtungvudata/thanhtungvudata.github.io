...
$$H_0: P(X < Y) = P(Y < X), \,\, \text{(two-sided test)}$$

or Group B tends to have larger values than Group A, i.e., 

$$H_0: P(X < Y) \leq 0.5, \,\, \text{(one-sided test) or}$$ 
...
$$U_A = \sum_{i=1}^{n_A} \sum_{j=1}^{n_B} ( \mathbb{I} (X_i < Y_j>) + 0.5 \mathbb{I} (X_i = Y_j>) )$$ 

$$U_B = \sum_{i=1}^{n_A} \sum_{j=1}^{n_B} ( \mathbb{I} (Y_j < X_i>) + 0.5 \mathbb{I} (Y_j = X_i>) )$$ 
